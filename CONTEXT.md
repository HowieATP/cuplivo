...
## Desktop Window Size (窗口尺寸)

- **Window floor**: The native minimum window size, `WindowSizeManager.minWindowWidth × minWindowHeight = 720×432` logical px, enforced on Windows and Linux (both window_manager and bitsdojo convert logical→physical via DPR, so the window can never get smaller than 720×432 logical). macOS has no native floor; the Dart-side guard in `DesktopHomePage` then applies — it CENTERS (and clips) the fixed-size shell when the window is below 720×432 logical, which on Windows/Linux is unreachable, so the guard only activates on macOS. Its constant MUST be read from `WindowSizeManager` — the two surfaces were once hardcoded in parallel; never hardcode a window floor in a page.
- **Quarter-snap (1/4 屏)**: Windows snap-to-quarter of the physical work area (e.g. 1280×800 physical on a 2560×1600 @ 175% display = ~731×457 logical). The acceptance target of issue #199. The floor must be ≤ quarter ÷ DPR, otherwise the window physically cannot reach a quarter layout — which is why the old 960×640 floor (1200×800 physical at 125% scaling) failed on scaled displays.
