...
- **Rendering state source (方向 C)**: The job owns the accumulated `streamedText`/reasoning buffer; the page REGISTERS its `StreamingContentNotifier` on the job (`job.uiNotifier`) while the child conversation is viewed, and `_run` updates it per chunk. There is no page-side chunk buffer — the previous page bridge kept a duplicate accumulation that had to be seeded manually. **Long-term (cuplivo v3)**: extract the page-independent "chunk → state + persistence" pipeline from `stream_controller` and have headless reuse it (方向 B), eliminating the headless functional subset entirely.
