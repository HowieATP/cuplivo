...
- **File content preview**: Tapping a file opens a reader mirroring `kelivo_read` semantics exactly (32 KB char budget, 32 MB size cap, binary rejection, truncation note) — one rule set, two surfaces. Common image extensions (png/jpg/gif/webp/svg) render inline; other types show a localized unsupported error. No in-page paging (YAGNI — the model has `kelivo_read`).
...
- **Download**: Per-file via `FilePicker.saveFile` (suggested name = basename), all platforms (desktop save dialog; mobile bytes-based save). No zip-bundle or share-sheet surface (YAGNI). Download is read-only, so it works on read-only external mounts too.
- **Mount rows & markers**: In the storage mounts panel, EVERY mount row tap → preview page (page parametrized by `{alias, path, readOnly}`); external rows carry `[铅笔]=edit` and `[垃圾桶]=remove` icons; the `@workspaces` row has no icons. Deletion markers (`workspaceFile`) are recorded ONLY for `@workspaces` — external-mount deletes are physical deletes with no tombstone (they never sync; a foreign alias in `deleted.json` would be unresolvable noise for peers). Read-only mounts hide upload/delete but keep preview + download.
...
- **kelivo_fetch**: The built-in in-memory MCP tool (server id `kelivo_fetch`, always present, not unbound-able). Returns token-conscious bounded text (default 5000 / max 20000 chars) with a `start_index` continuation hint. HTML → Markdown by default; `raw=true` returns raw source. Runs in-app on all platforms.
- **download_path**: Optional parameter — a FULL FILE wire path under `@workspaces/` ONLY (external mounts never; fetch is a built-in server, sandbox-only). Validated with the ADR 0022 rules: the shared `isSafeWireSegment` (imported from the filesystem server) guards segments; the rest is mirrored locally with `ArgumentError` (leading/trailing whitespace, missing `@workspaces/` prefix, trailing slash, backslash, `:`/NUL — fetch does NOT import `WirePathException`). ANY dot-prefixed segment is REJECTED (`.fetch_cache/`, `.git/`, `.hidden/`, …) — such paths are excluded from backup/sync and invisible to glob/grep, the opposite of a permanent download target (mirrors the trash resolver's dot-segment rule; users can still create dot-files explicitly via `kelivo_write`). Directories are not allowed — the path IS the complete target file; a download targeting an existing directory fails cleanly before any network I/O. Existing files are replaced atomically (backup staged INSIDE `.fetch_cache/` + rollback on rename failure — a crash between the two renames leaves only a cache-dir backup that eviction ages out; refusing overwrite would force the approval-gated `kelivo_delete` roundtrip for a routine refresh). When `download_path` is set, `max_length`/`start_index`/`raw` are ignored entirely — not even type-validated.
...
- **Timeouts**: The engine bounds both modes. TEXT mode: a single 60 s total timeout on `http.get` (connect + headers + body). DOWNLOAD mode: `client.send` (TCP connect + response headers) at 30 s and per-chunk inactivity at 60 s — a blackholed peer or a server that never sends headers fails instead of hanging the tool call. Both modes hold an explicit `HttpClient` and `close(force: true)` it on completion/failure — timed-out blackholed sockets are aborted, not left to the OS TCP timeout. CLIENT-SIDE cap: `mcp_client` wraps every `tools/call` in `requestTimeout` (global default 30 s, MCP-settings-configurable) — the built-in `@kelivo/fetch` server overrides it to a fixed 10 min (`mcp_provider`), because the engine's own timeouts bound every call and a tighter cap would report "Request timed out" while the fetch still completes in-isolate (file written, result discarded — the model believes it failed). A slow download on a user-lowered global timeout still exceeds that client cap; the file completes server-side and a retry re-downloads deterministically.
...
- **Page subscription**: On `switchConversationAnimated(convId)`, the page checks `HeadlessGenerationService.isActive(convId)`. If active, subscribes to `chunkStream(convId)` and drives the existing `StreamController` from those chunks. On navigate-away, unsubscribes (generation continues, persists to DB). On return, re-attaches or loads the finalized message. **Scope decision (wait-mode)**: implement this subscription — it does NOT exist in code today (`chunkStream()` has no callers; the child conversation appears frozen until completion). Real-time rendering uses the per-message `StreamingContentNotifier`; the DB write stays one-shot at completion (`_run` buffers). Without this, the panel's [查看子对话] leads to a frozen page.
...
- **MCP layer stays fast (MCP 层快速派发)**: Wait-mode must NOT block the JSON-RPC response — every `tools/call` await is bounded by the mcp_client `requestTimeout` (default 30s, global setting `mcp_request_timeout_ms_v1`). The engine returns the child conversation UUID immediately (identical to v1); the wait happens in the tool-handler layer, which awaits a completion future from the live store above the MCP boundary. This keeps MCP semantics, timeouts, and the vendored dependency untouched.
- **`kelivo_handoff_sync`**: The wait-mode tool, sibling of `kelivo_handoff` (same `assistant`/`task` parameters, same call-time validation, same conversation creation). The only semantic difference: the orchestrator's tool handler blocks until the child generation completes and returns the child's final output as the tool result. Returns structured JSON `{"conversation": "…", "status": "started"}` so the handler can extract the UUID without parsing prose. v1 `kelivo_handoff` is untouched (recorded tool events and existing prompts stay valid).
- **Full result, no truncation (全文直传)**: The tool result is the child's COMPLETE final output — no cap, no truncation marker. Rationale: sub-agent output is bounded by the child's `maxTokens` (unlike `kelivo_fetch`, whose input is unbounded — that is why fetch truncates and this tool does not); every output token is already paid for, and truncating would force the orchestrator to synthesize from a lossy prefix (or retry, which costs more). The existing `applyContextLimit` is the general safety net for pathological lengths — no subagent-specific cap mechanism (YAGNI).
- **Tool event records the full result automatically (统一记录)**: The `started` JSON is only the engine→handler internal response; the handler's return value (the child's full output) becomes the tool event `content` via the normal `toolResults` chunk path — no extra write-back mechanism needed. Known residual (out of scope): a cancelled/crashed outer stream leaves tool events at `content: null`, which `message_builder_service` treats as "pending" and drops the whole tool-call history on replay — a pre-existing general issue for ALL tools, not handoff-specific.
- **Live store (长任务 store)**: The root-level per-job state — `{jobId, status, lastStep, startedAt, completion future}` — that both the panel and the tool handler subscribe to. Deliberately shaped for generalization: any future long-running tool job (e.g. sandbox command execution with live stdout) is the same shape — MCP dispatches fast, the handler awaits, the UI watches the store. Do NOT pre-build abstractions for non-existent jobs.
- **子代理面板 (subagent panel)**: The transient live-status widget pinned above the parent conversation's input bar (collapsed pill ↔ expanded card, mirrors the Multi-AI badge row). Shows: phase (思考中/输出中/等待批准 — running states only), the child's last step (most recent tool call name + args summary; during pure streaming: "输出中…" + cumulative char count), elapsed time, and a [查看子对话] affordance. The panel exists ONLY while the job runs: on completion/error/cancel it disappears and the tool-call card takes over (full result / error marker). Full child output is NOT rendered in the panel — the forward chip / 查看子对话 leads to the child conversation. The tool-call card is the persistent anchor: for wait-mode it carries the child's FULL output as its result (no forward chip — the UUID regex of the v1 chip cannot parse the full text, and the panel + conversation-list badge are the navigation entry points); v1 `kelivo_handoff` cards keep the forward chip unchanged.
- **Parent attribution (父归属)**: The engine creates the child conversation with `setAsCurrent: false` — the delegating conversation stays `ChatService.currentConversationId`. The panel keys off that id, and nested handoffs attribute their parent to the real delegating conversation (not the last-created child); both break otherwise.
...
- **Approval/ask_user dual visibility (批准/询问双可见)**: Approval and ask_user requests raised inside a running subagent must be answerable from BOTH the parent panel and the child conversation. Feasible because `ToolApprovalService` / `AskUserInteractionService` are root ChangeNotifiers keyed by `toolCallId` — one completer per request, all rendering surfaces sync automatically. The panel auto-expands into an interactive approval/ask_user card when a pending request exists for the child. **Gap in v1**: subagent generations build their tool handler WITHOUT these services (`kelivo_subagent_server.dart` passes neither) — needsApproval MCP tools currently auto-execute in subagents and ask_user returns `ask_user_unavailable`. Wiring them is part of wait-mode.
