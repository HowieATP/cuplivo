...
3. **内置文件系统 MCP** — 通过内存 MCP 服务器读、写、正则查找本地文件；支持挂载本地目录，无需命令行，安全性优先；挂载后可在应用内文件浏览器中浏览，grep 搜索结果支持分页与上下文，支持查看代码架构（`kelivo_outline`）、下载互联网资源至工作区，长网页支持工作区缓存续读 (#173, #221, #222)。电脑端支持自定义内置工作区目录位置，工作区文件支持在外部打开与分享，文件预览增强，并修复空文件夹显示 (#242, #250)。
...
5. **TTS 音频：保存到本地 + 朗读选中文本** — 在浮窗播放器中保存云端生成的 TTS 音频至本地文件 (#131)；右键/长按选中的助手消息文本可朗读 (#130)。

6. **增强助手消息直接复制** — 朴素子序列 Markdown 复制 + 引用，快速提取消息内容 (#122)。

7. **多类别请求日志** — 请求日志现在覆盖 MCP、TTS、搜索服务，各自分类独立、开关与历史互不影响 (#162)。

8. **新装默认值优化** — 新安装默认启用 DeepSeek 供应商，默认关闭助手 temperature、top_p 等参数，默认开启请求日志并限制 50 MB；已进行相关配置的用户不受影响 (#240)。

9. **小地图消息级搜索** — 在对话小地图中搜索，筛选出匹配的消息并高亮匹配内容；桌面端浮层与移动端弹层共享同一套搜索逻辑 (#270)。
...
- **长消息流式渲染性能** — 为渲染和数据库写入添加智能节流，长消息流式输出更流畅 (#232)
