...
3. **Proactive care（Ta的来信）** — AI 可按设定时间主动向用户发送关怀消息（仅 Android）。
...
5. **子代理委派 (Handoff)** — 通过 MCP 工具将子任务委派给其他助手执行（即发即忘）。真正可返回结果的子代理计划在后续版本实现 (#140)。

6. **技能 (Skills)** — 从公开 GitHub 仓库导入技能、辅助文件工具。技能持久化存储在文件系统中，并包含在备份中。

7. **记忆模式切换 + 时间注入** — 每助手独立开关，保持系统提示词稳定以提升 API 缓存命中：记忆可在**自动注入**（每次对话自动注入记忆到系统提示词）与**按需工具**（通过 `read_memory` 工具按需读取记忆）间切换；时间注入在每条用户消息后追加缓存友好的时间戳，而非写入系统提示词。开启时间注入时，智能警告对话框会扫描系统提示词与记忆提示词中的易变变量 (#121)。
   - *提示*：为获得最佳缓存性能，建议关闭"最近对话参考"，切换到按需记忆模式并开启时间注入。

8. **搜索多 Key 轮换** — 为搜索服务配置多个 API Key，遇到限流自动轮换，提升有效限额 (#139)。

9. **SVG 预览** — 在 `svg` 代码块内联渲染 SVG 图表。

10. **对话侧边栏批量操作** — 在侧边栏中批量选择、删除或移动多个对话，提升对话管理效率 (#82)。

11. **删除恢复（回收站）** — 删除的对话进入回收站，可配置容量上限（默认 10 KB），防止误删；同步时携带删除标记，一端删除的内容会在另一端及时清除 (#137)。

12. **供应商级自定义 Headers/Body** — 每个供应商可附加自定义请求头与请求体字段 (#120)。

13. **存储空间管理** — 按时间或大小排序已存储文件，查找未被引用的图片/文件（孤儿），反向定位已存储文件属于哪条聊天记录 (#128)。

14. **增强助手消息直接复制** — 朴素子序列 Markdown 复制 + 引用，快速提取消息内容 (#122)。

15. **TTS 音频：保存到本地 + 朗读选中文本** — 在浮窗播放器中保存云端生成的 TTS 音频至本地文件 (#131)；右键/长按选中的助手消息文本可朗读 (#130)。

16. **每服务器心跳间隔** — 为每个 MCP 服务器单独配置心跳间隔，避免 429 限流 (#108)。

17. **PDF/Office 文件附件** — 支持直接上传 PDF、Word、Excel、PowerPoint 文档作为附件，并提供文档处理配置选项。

18. **美化请求日志** — 在日志查看器中拆分消息与配置，让请求体中的消息轮次更易读 (#127)。

19. **自定义动态主题色** — 通过色相拾取器选择自定义种子色，完全控制应用 accent 颜色 (#107)。

20. **桌面端 Markdown 表格工具栏** — 桌面端表格工具栏支持多格式复制（纯文本、HTML、LaTeX）(#109)。

21. **预设消息** — 预设消息在聊天列表中折叠到切换栏后；仅存在预设消息时阻止新建对话 (#116)。

22. **Thinking 开关** — 每助手独立开关，控制 summary/suggestion/compress/translate/OCR 模型的 thinking (#117)。
