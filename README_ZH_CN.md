...
3. **内置文件系统 MCP** — 通过内存 MCP 服务器读、写、正则查找本地文件；支持挂载本地目录，无需命令行，安全性优先 (#173)。
...
1. **设置中心改版** — 设置按分组组织（助手、外观与行为设置、提供商、辅助模型、SillyTavern功能、更多工具、数据设置、Cuplivo）；原设置页面保留在右上角旧版入口。

2. **阅读模式** — 较长的助手回答可开启阅读模式，降低阅读疲劳 (#160)。

3. **SVG 预览** — 在 `svg` 代码块内联渲染 SVG 图表。

4. **自定义动态主题色** — 通过色相拾取器选择自定义种子色，完全控制应用 accent 颜色 (#107)。

5. **美化请求日志** — 在日志查看器中拆分消息与配置，让请求体中的消息轮次更易读 (#127)。

6. **桌面端 Markdown 表格工具栏** — 桌面端表格工具栏支持多格式复制（纯文本、HTML、LaTeX）(#109)。

7. **预设消息** — 预设消息在聊天列表中折叠到切换栏后；仅存在预设消息时阻止新建对话 (#116)。
