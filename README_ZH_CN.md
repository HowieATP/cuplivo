...
1. **Linux 沙箱** — 运行完整 Linux 沙箱：**Android** 可在界面选择发行版，**iOS** 通过 iSH 运行沙箱；完成相应配置的用户可以执行命令行工具。Android 还可在工作区设置中打开类 Termux 的交互式终端，与模型 Shell 工具相互独立。Android 还可通过 SAF + 定时同步将外部目录挂载到工作区。
