...
4. **记忆模式切换 + 时间注入** — 每助手独立开关，保持系统提示词稳定以提升 API 缓存命中：记忆可在**自动注入**（每次对话自动注入记忆到系统提示词）与**按需工具**（通过 `read_memory` 工具按需读取记忆）间切换；时间注入在每条用户消息后追加缓存友好的时间戳，而非写入系统提示词。开启时间注入时，智能警告对话框会扫描系统提示词与记忆提示词中的易变变量 (#121)。
...
5. **每服务器心跳间隔** — 为每个 MCP 服务器单独配置心跳间隔，避免 429 限流 (#108)。

6. **PDF/Office 文件附件** — 支持直接上传 PDF、Word、Excel、PowerPoint 文档作为附件，并提供文档处理配置选项。

7. **Thinking 开关** — 每助手独立开关，控制 summary/suggestion/compress/translate/OCR 模型的 thinking (#117)。

#### 实用能力增强

1. **对话侧边栏批量操作** — 在侧边栏中批量选择、删除或移动多个对话，提升对话管理效率 (#82)。

2. **存储空间管理** — 按时间或大小排序已存储文件，查找未被引用的图片/文件（孤儿），反向定位已存储文件属于哪条聊天记录 (#128)。

3. **TTS 音频：保存到本地 + 朗读选中文本** — 在浮窗播放器中保存云端生成的 TTS 音频至本地文件 (#131)；右键/长按选中的助手消息文本可朗读 (#130)。

4. **增强助手消息直接复制** — 朴素子序列 Markdown 复制 + 引用，快速提取消息内容 (#122)。

5. **多类别请求日志** — 请求日志现在覆盖 MCP、TTS、搜索服务，各自分类独立、开关与历史互不影响 (#162)。
