...
1. **Linux 沙箱** — 运行完整 Linux 沙箱：**Android** 可在界面选择发行版，**iOS** 通过 iSH 运行沙箱；完成相应配置的用户可以执行命令行工具 (#301)。Android 还可在工作区设置中打开类 Termux 的交互式终端，与模型 Shell 工具相互独立 (#428)。

2. **Proactive care（Ta的来信）** — AI 可按设定时间主动向用户发送关怀消息（仅 Android）。
   - 仅 Android：后台闹钟 + 通知渠道，闹钟在强制停止后可恢复
   - *提示*：在助手设置的"Ta的来信"标签页中开启

3. **多助手群聊** — 导演模型编排的群聊：后台导演模型决定由哪位助手发言，成员在共享对话中聊天，各自持有私有上下文 (#150)。

4. **内置文件系统 MCP** — 通过内存 MCP 服务器读、写、正则查找本地文件；支持挂载本地目录，无需命令行，安全性优先；挂载后可在应用内文件浏览器中浏览，grep 搜索结果支持分页与上下文，支持查看代码架构（`kelivo_outline`）、下载互联网资源至工作区，长网页支持工作区缓存续读 (#173, #221, #222)。电脑端支持自定义内置工作区目录位置，工作区文件支持在外部打开与分享，文件预览增强，并修复空文件夹显示 (#242, #250)。
...
1. **子代理委派 (Handoff)** — 通过 MCP 工具将子任务委派给其他助手执行：即发即忘模式用于后台任务；等待模式下子代理完成后会将结果返回给主代理进一步处理，主对话附有实时进度看板，支持同轮并发调用 (#140, #251)。

2. **技能 (Skills)** — 从公开 GitHub 仓库导入技能，并提供辅助文件工具。技能持久化存储在文件系统中，并包含在备份中。v3 新增分类、总开关与对话级技能入口 (#161)；内置工具让助手可直接导入与创建技能 (#319)。

#### API 调控能力增强

1. **绘图参数面板** — 添加 OpenAI 图像 API 可视化配置，快捷控制质量、尺寸/宽高比、输出格式、数量等参数 (#248)。

2. **OAuth 账户登录** — Grok xAI (#164) 与 OpenAI Codex (#157) 支持设备码登录；MCP 支持 OAuth v2 自动流程：授权服务器发现、动态客户端注册与回环回调 (#156)。

3. **MCP 工具结果图回传** — 将 MCP 工具返回的图片回传给 LLM 供应商，让模型能看到工具输出 (#159)。

4. **每助手 OCR 模式** — 新增"智能" OCR 模式：模型有视觉能力时关闭 OCR，无视觉能力时开启 OCR；每助手可选自动/始终/关闭 (#171)。

5. **PDF/Office 文件附件** — 支持直接上传 PDF、Word、Excel、PowerPoint 文档作为附件，并提供文档处理配置选项。

#### 实用能力增强

1. **输入草稿** — 输入内容会保存为草稿，应用重启后自动恢复上一次未发送的内容 (#246)。

2. **增强助手消息直接复制** — 朴素子序列 Markdown 复制 + 引用，快速提取消息内容 (#122)。

3. **数学公式导出** — 块级公式支持复制为 LaTeX / 复制为 PNG / 下载为 PNG (#345)。

4. **AI 日志分析** — 在请求日志界面一键让 AI 分析已脱敏日志 (#390)。

5. **工具中心 (Tools Hub)** — 将 MCP 服务器、本地工具、工作区管理（含挂载、安卓终端启动）统一到原 MCP 入口，供对话时快捷调整 (#491)。

#### 界面渲染功能增强

1. **HTML 代码块内联预览** — HTML 代码块现在会在聊天列表中内联渲染为可交互的预览块，助手只需输出 HTML 代码块即可实现丰富排版（如角色扮演场景）(#174, #203)。

2. **阅读模式** — 较长的助手回答可开启阅读模式，降低阅读疲劳 (#160)。

3. **SVG 预览** — 在 `svg` 代码块内联渲染 SVG 图表。

4. **桌面端 Markdown 表格工具栏** — 桌面端表格工具栏支持多格式复制（纯文本、HTML、LaTeX）(#109)。

5. **预设消息** — 预设消息在聊天列表中折叠到切换栏后；仅存在预设消息时阻止新建对话 (#116)。
