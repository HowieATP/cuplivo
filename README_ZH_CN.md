...
3. **搜索多 Key 轮换** — 为搜索服务配置多个 API Key，遇到限流自动轮换，提升有效限额 (#139)。

4. **MCP 工具结果图回传** — 将 MCP 工具返回的图片回传给 LLM 供应商，让模型能看到工具输出 (#159)。

5. **供应商级自定义 Headers/Body** — 每个供应商可附加自定义请求头与请求体字段 (#120)。

6. **每助手 OCR 模式** — 新增"智能" OCR 模式：模型有视觉能力时关闭 OCR，无视觉能力时开启 OCR；每助手可选自动/始终/关闭 (#171)。

7. **每服务器心跳间隔** — 为每个 MCP 服务器单独配置心跳间隔，避免 429 限流 (#108)。

8. **PDF/Office 文件附件** — 支持直接上传 PDF、Word、Excel、PowerPoint 文档作为附件，并提供文档处理配置选项。

9. **Thinking 开关** — 每助手独立开关，控制 summary/suggestion/compress/translate/OCR 模型的 thinking (#117)。
...
4. **输入草稿** — 输入内容会保存为草稿，应用重启后自动恢复上一次未发送的内容 (#246)。

5. **增强助手消息直接复制** — 朴素子序列 Markdown 复制 + 引用，快速提取消息内容 (#122)。

6. **小地图消息级搜索** — 在对话小地图中搜索，筛选出匹配的消息并高亮匹配内容；桌面端浮层与移动端弹层共享同一套搜索逻辑 (#270)。

7. **批量导出对话** — 一次性将多个对话导出为 Markdown (#276, #305)。

8. **数学公式导出** — 块级公式支持复制为 LaTeX / 复制为 PNG / 下载为 PNG (#345)。

9. **AI 日志分析** — 在请求日志界面一键让 AI 分析已脱敏日志 (#390)。

10. **工具中心 (Tools Hub)** — 将 MCP 服务器、本地工具、工作区管理（含挂载、安卓终端启动）统一到原 MCP 入口，供对话时快捷调整 (#491)。

#### 界面渲染功能增强

1. **透明块渲染** — 代码块、表格块、`<details>` 块底色改为半透明，在有背景图时观感更佳 (#452)。

2. **HTML 代码块内联预览** — HTML 代码块现在会在聊天列表中内联渲染为可交互的预览块，助手只需输出 HTML 代码块即可实现丰富排版（如角色扮演场景）(#174, #203)。

3. **阅读模式** — 较长的助手回答可开启阅读模式，降低阅读疲劳 (#160)。

4. **SVG 预览** — 在 `svg` 代码块内联渲染 SVG 图表。
...
- **Token 数量统计** — 多轮工具调用的助手消息 Token 数量按总和计算，应用内统计不再低估 Token 消耗 (#247)
- 优化标题生成逻辑（首次失败自动重试）
