...
6. **应用内更新提醒 + 按架构匹配下载链接** — 启动时自动检查 GitHub Releases，发现新版本后在侧边栏展示更新入口，并附更新日志（与安卓更新提示同一套 UI）。iOS 端按设备架构匹配下载链接（arm64 真机 / x86_64 模拟器），让使用未上架 .ipa 的用户始终下载到与自己设备匹配的安装包 (#233)。
