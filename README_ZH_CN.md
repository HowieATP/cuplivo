...
1. **增量备份 & 局域网同步** — 仅上传自某个选定日期以来的对话和消息，以及附件等；在局域网下快捷同步两台设备的状态。避免了每次同步都需要走公网传输极大 zip 文件的麻烦。
...
2. **删除恢复（回收站）** — 删除的对话进入回收站，可配置容量上限（默认 10 KB），防止误删；同步时携带删除标记，一端删除的内容会在另一端及时清除。

3. **从 RikkaHub 导入** — 通过迁移网站将 RikkaHub 备份转换为 Cuplivo 兼容的备份，再通过"备份文件导入"导入即可。

#### 提升对话体验的招牌能力

1. **Linux 沙箱** — 运行完整 Linux 沙箱：**Android** 可在界面选择发行版，**iOS** 通过 iSH 运行沙箱；完成相应配置的用户可以执行命令行工具。Android 还可在工作区设置中打开类 Termux 的交互式终端，与模型 Shell 工具相互独立。
...
4. **内置文件系统 MCP** — 通过内存 MCP 服务器读、写、正则查找本地文件；支持挂载本地目录，无需命令行，安全性优先；挂载后可在应用内文件浏览器中浏览，grep 搜索结果支持分页与上下文，支持查看代码架构、下载互联网资源至工作区，长网页支持工作区缓存续读。电脑端支持自定义内置工作区目录位置，工作区文件支持在外部打开与分享。
...
3. **MCP 工具结果图回传** — 将 MCP 工具返回的图片回传给 LLM 供应商，让模型能看到工具输出。

4. **OCR 智能模式** — 新增"智能" OCR 模式：模型有视觉能力时关闭 OCR，无视觉能力时开启 OCR；每助手可选自动/始终/关闭。

5. **PDF/Office 文件附件** — 支持直接上传 PDF、Word、Excel、PowerPoint 文档作为附件，并提供文档处理配置选项。

#### 实用能力增强

1. **输入草稿** — 输入内容会保存为草稿，应用重启后自动恢复上一次未发送的内容。

2. **增强助手消息直接复制** — 朴素子序列 Markdown 复制 + 引用，快速提取消息内容。

3. **数学公式导出** — 块级公式支持复制为 LaTeX / 复制为 PNG / 下载为 PNG。

4. **AI 日志分析** — 在请求日志界面一键让 AI 分析已脱敏日志。

5. **工具中心 (Tools Hub)** — 将 MCP 服务器、本地工具、工作区管理（含挂载、安卓终端启动）统一到原 MCP 入口，供对话时快捷调整。

#### 界面渲染功能增强

1. **HTML 代码块内联预览** — HTML 代码块现在会在聊天列表中内联渲染为可交互的预览块，助手只需输出 HTML 代码块即可实现丰富排版 (如角色扮演场景)。
...
- 图片分享：导出图片强制 8 位 sRGB 读回，修复自 v1.1.16 以来 iOS 表格底色异常的问题
