 ...
 * (minutes before start, >= 1) and creates a CalendarContract.Reminders entry
 * — a deliberate Cuplivo extension over upstream Kelivo v1.2.1, addressing
 * Chevey339/kelivo#920 (created events carried no notification).
 */
class DeviceLocalToolsHandler(private val activity: Activity) {
    companion object {
        ...
        private const val INVALID_REMINDER_VALUE = -1
    }
    ...
    private fun createCalendarEvent(params: JSONObject): String {
        ...
        val reminderMinutes = parseReminderMinutes(params)
        if (reminderMinutes == INVALID_REMINDER_VALUE) {
            return errorPayload(
                "INVALID_REMINDER",
        ...
        if (reminderMinutes != null) {
            ...
            // Roll back the event if the reminder row cannot be created so the
            // tool never reports success for a half-created event.
            if (activity.contentResolver.insert(CalendarContract.Reminders.CONTENT_URI, reminderValues) == null) {
                activity.contentResolver.delete(
                    ContentUris.withAppendedId(CalendarContract.Events.CONTENT_URI, eventId),
                    null,
                    null,
                )
                return errorPayload("INSERT_FAILED", "Event created but reminder could not be inserted.")
            }
        }
    ...
    /** Parses reminder_minutes; null when omitted or JSON null, [INVALID_REMINDER_VALUE] when invalid. */
    private fun parseReminderMinutes(params: JSONObject): Int? {
        if (!params.has("reminder_minutes")) return null
        val raw = params.opt("reminder_minutes") ?: return null
        if (raw == JSONObject.NULL) return null
        val minutes = when (raw) {
            is Int -> raw
            is Long -> raw.toInt()
            is Double -> if (raw == raw.toLong().toDouble()) raw.toInt() else INVALID_REMINDER_VALUE
            is String -> raw.toIntOrNull() ?: INVALID_REMINDER_VALUE
            else -> INVALID_REMINDER_VALUE
        }
        if (minutes == INVALID_REMINDER_VALUE || minutes < 1) return INVALID_REMINDER_VALUE
        return minutes
    }
