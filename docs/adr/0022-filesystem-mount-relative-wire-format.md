...
- **Model-facing presentation (workspace local tools)**: the workspace local-tools surface presents a single model-facing vocabulary — `/workspace/rel/path` for the bound workspace root (same directory the Linux sandbox shell's guest cwd `/workspace` resolves to) — and translates it to the canonical wire form at the tool boundary (`lib/core/services/workspace/workspace_path_presentation.dart`, wired in `WorkspaceToolsService`). Inputs accept ONLY `/workspace/...`; canonical `@alias/...` is rejected with a hint. Canonical wire paths remain the persisted identity format (deletion markers, backups), and the engine's `pathPresenter` is display-only: the deletion marker callback always receives the canonical path. Recorded tool calls created before this presentation existed carry `@alias/...` args and now fail input validation on replay (strict mode, accepted breakage); no identity data migration is needed.
