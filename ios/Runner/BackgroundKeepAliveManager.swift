...
//
//   2. Location keep-alive — coarse CLLocationManager updates (30s
...
final class BackgroundKeepAliveManager: NSObject, CLLocationManagerDelegate {
  ...
  private(set) var locationHeartbeatInterval: TimeInterval = 30.0
  ...
  private func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "configure":
      ...
      configure(
        ...
        locationHeartbeatInterval: args["locationHeartbeatInterval"] as? TimeInterval ?? 30.0
      )
      result(true)
    case "beginSession":
      sessionActive = true
  ...
  /// Whether a generation session is currently open on the Dart side.
  /// Used by AppDelegate to avoid re-arming BGTask scheduling when no
  /// generation work is actually running.
  var isGenerationSessionActive: Bool {
    sessionActive
  }
  ...
  private func startSilentAudio(reason: String = "evaluate") {
    ...
    // Pure-silence buffer: 16 kHz mono is more than enough to keep the
    // audio session alive and renders at a fraction of the CPU cost of
    // 44.1 kHz.
  ...
  private func evaluateLocationUpdates() {
    // Audio is the primary leg; location only backs it up when the silent
    // audio leg is unavailable (not enabled / failed to activate / suspended
    // by user media), so we never run both radios at once.
    ...
    if shouldUpdate && !locationUpdating {
      ...
      let timer = Timer(timeInterval: locationHeartbeatInterval, repeats: true) { [weak self] _ in
        Task { @MainActor [weak self] in
