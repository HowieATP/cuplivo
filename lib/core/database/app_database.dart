...
class AppDatabase extends _$AppDatabase {
  ...
  /// Add a column only when missing. Prefer this over bare addColumn+catch so we
  /// never advance past a migration while leaving the schema incomplete.
  ...
  /// ALTER TABLE steps were skipped/failed (silent catch). Runs every open.
  Future<void> _healSchemaIfNeeded() async {
    ...
    // --- group chat tables (v13) ---
    final m = createMigrator();
    if (!await _hasTable('group_chat_rows')) {
      try {
        await m.createTable(groupChatRows);
      } catch (_) {}
    }
    if (!await _hasTable('group_chat_member_rows')) {
      try {
        await m.createTable(groupChatMemberRows);
      } catch (_) {}
    }
    if (!await _hasTable('director_message_rows')) {
      try {
        await m.createTable(directorMessageRows);
      } catch (_) {}
    }
    if (!await _hasTable('deleted_record_rows')) {
      try {
        await m.createTable(deletedRecordRows);
      } catch (_) {}
    }
    if (!await _hasTable('deletion_marker_rows')) {
      try {
        await m.createTable(deletionMarkerRows);
      } catch (_) {}
    }
  ...
  MigrationStrategy get migration => MigrationStrategy(
    beforeOpen: (details) async {
      ...
      // Always self-heal: user_version may already be 12/13 while individual
      // addColumn steps were skipped (empty catch) during an earlier upgrade.
