...
class _MarkdownWithCodeHighlightState extends State<MarkdownWithCodeHighlight> {
  ...
  // debounce early. Below this, a full markdown re-parse per stream tick is
  // cheap; above it, renders are coalesced to the 120ms cadence. The debounce
  // only keys off the LIVE TAIL length: committed stable blocks are cached
  // and never re-parse.
  ...
  /// re-parse, re-layout or re-paint it. The cache is dropped when the
  /// stream shrinks, a regex transform changes the committed prefix, the
  /// theme changes, or streaming ends.
  ...
  Widget build(BuildContext context) {
    ...
    Widget buildMarkdownFor(String text, {required bool streaming}) {
      return GptMarkdown(
        key: ValueKey('$markdownThemeKey-${streaming ? 'stream' : 'final'}'),
        text,
        style: baseTextStyle,
    ...
    // still-growing tail. Committed blocks are wrapped in RepaintBoundary and
    // never re-parsed or re-laid-out by later tokens.
    if (widget.streaming) {
      _syncStableBlocks(
        normalized,
