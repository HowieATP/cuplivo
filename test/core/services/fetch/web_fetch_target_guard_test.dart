...
void main() {
  ...
  group('built-in fetch SSRF blocking', () {
    ...
    setUp(() async {
      ...
      wsDir = await Directory.systemTemp.createTemp('web_fetch_ssrf_test');
    ...
    tearDown(() async {
      ...
      if (wsDir.existsSync()) await wsDir.delete(recursive: true);
    });

    test('text-mode fetch rejects loopback target', () async {
      final result = await _callFetch(wsDir, baseUri.resolve('/secret'));
      expect(result['isError'], isTrue);
      expect(_resultText(result), contains('not an allowed web_fetch target'));
    });

    test('download-mode fetch rejects loopback target', () async {
      final result = await _callFetch(
        wsDir,
        baseUri.resolve('/secret'),
        arguments: const {'download_path': '@workspaces/secret.txt'},
      );
      expect(result['isError'], isTrue);
      expect(_resultText(result), contains('not an allowed web_fetch target'));
    });
...
Future<Map<String, dynamic>> _callFetch(
  ...
  try {
    ...
    return _ok(result.downloadPath ?? result.content ?? '');
  } catch (e) {
